 ...
 @if (session('alert'))
     <script>
         const alert = {!! json_encode(session('alert.config')) !!};
 ...
 <script type="text/javascript">
     jQuery(document).ready(function($) {
         ...
         @if ($crud->getOperationSetting('resetButton') ?? true)
             ...
             var crudTableResetButton =
                 '<a href="{{ url($crud->route) }}" class="ml-1 ms-1" id="crudTable_reset_button">{{ trans('backpack::crud.reset') }}</a>';
     ...
     function formatActionColumnAsDropdown() {
         ...
         $('#crudTable tbody tr').each(function(i, tr) {
             ...
             actionCell.prepend(
                 '<a class="btn btn-sm px-2 py-1 btn-outline-primary dropdown-toggle actions-buttons-column" href="#" data-toggle="dropdown" data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false">{{ trans('backpack::crud.actions') }}</a>'
             );
         });
     }
 </script>
